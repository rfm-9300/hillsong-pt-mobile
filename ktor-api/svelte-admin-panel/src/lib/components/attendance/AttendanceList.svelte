        ...
        <div class="flex flex-col sm:flex-row gap-2">
            ...
            <div class="relative">
                <div class="relative group">
                    ...
                    <div class="absolute hidden group-hover:block mt-1 w-full bg-white border border-gray-300 rounded-md shadow-lg z-10">
                        <div class="py-1">
                            <button 
                                class="w-full text-left px-4 py-2 text-sm hover:bg-gray-100 flex items-center gap-2"
                                on:click={() => { filterStatus = 'ALL'; handleStatusFilterChange({ target: { value: 'ALL' } }); }}
                            >
                                <svg class="h-4 w-4 text-gray-500" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 4a1 1 0 011-1h16a1 1 0 011 1v2.586a1 1 0 01-.293.707l-6.414 6.414a1 1 0 00-.293.707V17l-4 4v-6.586a1 1 0 00-.293-.707L3.293 7.293A1 1 0 013 6.586V4z" />
                                </svg>
                            ...
                            <button 
                                class="w-full text-left px-4 py-2 text-sm hover:bg-gray-100 flex items-center gap-2"
                                on:click={() => { filterStatus = AttendanceStatus.CHECKED_IN; handleStatusFilterChange({ target: { value: AttendanceStatus.CHECKED_IN } }); }}
                            >
                                <StatusBadge status={AttendanceStatus.CHECKED_IN} size="sm" />
                            </button>
                            <button 
                                class="w-full text-left px-4 py-2 text-sm hover:bg-gray-100 flex items-center gap-2"
                                on:click={() => { filterStatus = AttendanceStatus.CHECKED_OUT; handleStatusFilterChange({ target: { value: AttendanceStatus.CHECKED_OUT } }); }}
                            >
                                <StatusBadge status={AttendanceStatus.CHECKED_OUT} size="sm" />
                            </button>
                            <button 
                                class="w-full text-left px-4 py-2 text-sm hover:bg-gray-100 flex items-center gap-2"
                                on:click={() => { filterStatus = AttendanceStatus.EMERGENCY; handleStatusFilterChange({ target: { value: AttendanceStatus.EMERGENCY } }); }}
                            >
                                <StatusBadge status={AttendanceStatus.EMERGENCY} size="sm" />
                            </button>
                            <button 
                                class="w-full text-left px-4 py-2 text-sm hover:bg-gray-100 flex items-center gap-2"
                                on:click={() => { filterStatus = AttendanceStatus.NO_SHOW; handleStatusFilterChange({ target: { value: AttendanceStatus.NO_SHOW } }); }}
                            >
                        ...
                        {#each filteredRecords as record (record.attendance.id)}
                            <tr class="hover:bg-gray-50 relative">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap" data-label="Check-in Time">
                                    <div class="text-sm text-gray-900">{formatDate(record.attendance.checkInTime)}</div>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap" data-label="Check-out Time">
                                    {#if record.attendance.checkOutTime}
                                        <div class="text-sm text-gray-900">{formatDate(record.attendance.checkOutTime)}</div>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium relative" data-label="Actions">
                                    <div class="flex justify-end space-x-2">
                                        {#if record.attendance.status === AttendanceStatus.CHECKED_IN}
                                            <Button 
                                                ...
                                                disabled={operationsInProgress.has(`checkout-${record.attendance.id}`) || operationsInProgress.has(`status-${record.attendance.id}`)}
                                            >
                                                Check Out
                                            </Button>
                                        {/if}
                                        <div class="relative group">
                                            ...
                                            <div class="absolute hidden group-hover:block right-0 mt-1 w-48 bg-white border border-gray-300 rounded-md shadow-lg z-10">
                                                <div class="py-1">
                                                    <button 
                                                        class="w-full text-left px-4 py-2 text-sm hover:bg-gray-100 flex items-center gap-2 {operationsInProgress.has(`checkout-${record.attendance.id}`) || operationsInProgress.has(`status-${record.attendance.id}`) ? 'opacity-50 cursor-not-allowed' : ''}"
                                                        on:click={() => openStatusUpdateModal(record)}
                                                        disabled={operationsInProgress.has(`checkout-${record.attendance.id}`) || operationsInProgress.has(`status-${record.attendance.id}`)}
                                                    >
                                                    ...
                                                    <button 
                                                        class="w-full text-left px-4 py-2 text-sm hover:bg-gray-100 flex items-center gap-2"
                                                        on:click={() => handleStatusUpdate(record, AttendanceStatus.EMERGENCY)}
                                                    ...
                                                    <button 
                                                        class="w-full text-left px-4 py-2 text-sm hover:bg-gray-100 flex items-center gap-2"
                                                        on:click={() => handleStatusUpdate(record, AttendanceStatus.NO_SHOW)}
                    ...
                    <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                        <div>
                            <p class="text-sm text-gray-700">
                                Showing <span class="font-medium">{(currentPage - 1) * itemsPerPage + 1}</span> to <span class="font-medium">{Math.min(currentPage * itemsPerPage, attendanceRecords.length)}</span> of <span class="font-medium">{attendanceRecords.length}</span> results
                            </p>
                        </div>
                        <div>
                            <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
                                ...
                                {#each Array(totalPages) as _, i}
                                    <button
                                        class="relative inline-flex items-center px-4 py-2 border border-gray-300 bg-white text-sm font-medium {currentPage === i + 1 ? 'text-indigo-600 bg-indigo-50' : 'text-gray-700 hover:bg-gray-50'}"
    ...
    >
        {#if selectedRecord}
            <div class="mb-4">
                ...
                <p class="text-sm text-gray-500">Current Status: <StatusBadge status={selectedRecord.attendance.status} size="sm" /></p>
            </div>
            ...
            <StatusUpdateInterface
                ...
                on:update={handleStatusUpdateFromModal}
                on:cancel={() => showStatusUpdateModal = false}
                on:complete={() => showStatusUpdateModal = false}
            />
        {/if}
    </Modal>
</div>
