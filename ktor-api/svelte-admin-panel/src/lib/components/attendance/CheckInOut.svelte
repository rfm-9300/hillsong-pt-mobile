<script>
	...
	function selectAttendee(attendee) {
		...
		if (
			searchQuery.trim() &&
			!searchHistory.some((item) => item.id === attendee.id && item.type === selectedAttendeeType)
		) {
	...
	async function checkIn() {
		...
		while (attempts < maxAttempts) {
			try {
				...
				try {
							...
							if (selectedAttendeeType === 'USER') {
								switch (eventType) {
									case EventType.EVENT:
										isAlreadyCheckedIn = await attendanceService.isUserCheckedInToEvent(
											eventId,
											selectedAttendee.id
										);
										break;
									case EventType.SERVICE:
										isAlreadyCheckedIn = await attendanceService.isUserCheckedInToService(
											eventId,
											selectedAttendee.id
										);
										break;
								...
								isAlreadyCheckedIn = await attendanceService.isKidCheckedInToKidsService(
									eventId,
									selectedAttendee.id
								);
							}
					...
					if (isAlreadyCheckedIn) {
						throw new Error(
							`${selectedAttendee.name} is already checked in to this ${eventType.toLowerCase()}`
						);
					}
				...
				const checkInPromise = attendanceService.checkInByEventType(
					eventType,
					eventId,
					checkInRequest
				);
		...
		if (
			...
			error =
				'Unable to complete check-in after multiple attempts. The server might be experiencing issues. Please try again later.';
		}
					...
					const statusCheckPromise = new Promise(async (resolve, reject) => {
						try {
							...
							if (selectedAttendeeType === 'USER') {
								switch (eventType) {
									case EventType.EVENT:
										result = await attendanceService.isUserCheckedInToEvent(
											eventId,
											selectedAttendee.id
										);
										break;
									case EventType.SERVICE:
										result = await attendanceService.isUserCheckedInToService(
											eventId,
											selectedAttendee.id
										);
										break;
								...
								result = await attendanceService.isKidCheckedInToKidsService(
									eventId,
									selectedAttendee.id
								);
							}
	...
	<Card>
					...
					<div>
						...
						{#if validationErrors.attendee === 'already-checked-in'}
							<p class="text-sm mt-1">
								This attendee is already checked in. Please select a different attendee or check
		...
		{#if showQuickSelect}
			<div class="mb-4 bg-gray-50 p-3 rounded-lg border border-gray-200">
				...
				<div class="flex border-b border-gray-200 mb-3">
					<button
						class="py-2 px-4 text-sm font-medium {selectedTab === 'recent'
					...
					<button
						class="py-2 px-4 text-sm font-medium {selectedTab === 'frequent'
							? 'text-indigo-600 border-b-2 border-indigo-600'
							: 'text-gray-500 hover:text-gray-700'}"
								...
								>
									<div class="flex flex-col items-start">
										...
										<div class="flex items-center">
											<span
												class="text-xs bg-indigo-100 text-indigo-800 px-1.5 py-0.5 rounded-full"
											>
		...
		<div class="mb-4 relative">
			...
			{#if showSearchHistory && searchHistory.length > 0 && !searchResults.length}
				...
				>
					<div
						class="flex justify-between items-center px-3 py-2 bg-gray-50 border-b border-gray-200"
					>
							...
							<li>
								...
								>
									<div class="flex justify-between items-center">
										...
										<span
											class="text-xs px-2 py-0.5 rounded-full {item.type === 'USER'
				...
				>
					<ul class="py-1">
						{#each searchResults as result, index}
							<li>
								<button
									class="w-full text-left px-4 py-2 {index === activeSearchIndex
										? 'bg-indigo-50'
					...
					<div>
						...
						<p class="text-sm text-gray-600">
							{selectedAttendeeType === 'USER'
								...
								: `Parent: ${selectedAttendee.parentName}`}
						</p>
					</div>
					<button
		...
		<div class="mb-4">
			...
			{#if notes && notes.length > 450}
				<div class="mt-1 text-xs {notes.length > 500 ? 'text-red-600' : 'text-amber-600'}">
					{notes.length}/500 characters {notes.length > 500
						? '(limit exceeded)'
						: '(approaching limit)'}
				</div>
			{/if}
