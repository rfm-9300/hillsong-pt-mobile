...
class CheckInRequestController(private val checkInRequestService: CheckInRequestService) {

    /**
     * Create a new check-in request for a child.
     *
     * Parents use this endpoint to generate a QR code for check-in. The QR code contains a secure
    ...
    fun createCheckInRequest(
        ...
        return try {
            ...
            org.slf4j.LoggerFactory.getLogger(this::class.java)
                    .error("Security error creating check-in request: ${e.message}", e)
            ResponseEntity.status(HttpStatus.FORBIDDEN)
                    .body(ApiResponse(success = false, message = e.message ?: "Access denied"))
        } catch (e: IllegalArgumentException) {
            org.slf4j.LoggerFactory.getLogger(this::class.java)
                    .error("Validation error creating check-in request: ${e.message}", e)
            ResponseEntity.badRequest()
                    .body(ApiResponse(success = false, message = e.message ?: "Invalid request"))
        } catch (e: IllegalStateException) {
            org.slf4j.LoggerFactory.getLogger(this::class.java)
                    .error("State error creating check-in request: ${e.message}", e)
            ResponseEntity.badRequest()
                    .body(
                            ApiResponse(
                                    success = false,
            ...
            org.slf4j.LoggerFactory.getLogger(this::class.java)
                    .error("Unexpected error creating check-in request: ${e.message}", e)
            ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(
                            ApiResponse(
                                    success = false,
                                    message =
                                            "An error occurred while creating the check-in request"
                            )
                    )
        }
    }
            ...
            ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(
                            ApiResponse(
                                    success = false,
                                    message =
                                            "An error occurred while retrieving the check-in request"
                            )
                    )
        }
    }
     ...
     *
     * Staff use this endpoint after verifying the child's identity and information to complete the
     * check-in process. This creates an attendance record.
     *
     * @param token The token from the scanned QR code
    ...
    fun approveCheckIn(
        ...
        return try {
            ...
            val response =
                    checkInRequestService.approveCheckInRequest(
                            token = token,
            ...
            val statusCode =
                    when {
                        e.message?.contains("capacity", ignoreCase = true) == true ->
                                HttpStatus.CONFLICT
                        e.message?.contains("already checked in", ignoreCase = true) == true ->
                                HttpStatus.CONFLICT
     ...
     *
     * Staff use this endpoint when they cannot approve a check-in. A reason must be provided which
    ...
    fun rejectCheckIn(
        ...
        return try {
            val staffUserId = authentication.getCurrentUserId()
            val response =
                    checkInRequestService.rejectCheckInRequest(
                            token = token,
                            staffUserId = staffUserId,
            ...
            ResponseEntity.ok(
                    ApiResponse(success = true, message = "Check-in rejected", data = response)
            )
        } catch (e: IllegalArgumentException) {
            ResponseEntity.badRequest()
                    .body(
     ...
     *
     * Parents use this endpoint to cancel a check-in request they no longer need. Only pending
            ...
            ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(
                            ApiResponse(
                                    success = false,
                                    message =
                                            "An error occurred while cancelling the check-in request"
                            )
                    )
        }
    }
     ...
     *
     * Parents use this endpoint to view all their pending check-in requests across all their
     * children.
     *
     * @param authentication Current authenticated user (parent)
            ...
            ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(
                            ApiResponse(
                                    success = false,
                                    message =
                                            "An error occurred while retrieving active check-in requests"
                            )
                    )
        }
    }
