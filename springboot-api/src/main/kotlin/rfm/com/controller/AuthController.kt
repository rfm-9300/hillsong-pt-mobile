...
/**
 * REST Controller for authentication endpoints Handles user login, signup, verification, password
 * reset, and OAuth2 authentication
 */
@RestController
...
class AuthController(private val userService: UserService) {
    ...
    /** Email verification endpoint via GET (for email links) GET /api/auth/verify?token=... */
    @GetMapping("/verify")
    ...
    /** Password reset request endpoint POST /api/auth/forgot-password */
    ...
    /** Google OAuth2 login endpoint POST /api/auth/google-login */
    ...
    /** Facebook OAuth2 login endpoint POST /api/auth/facebook-login */
    @PostMapping("/facebook-login")
    fun facebookLogin(
            @Valid @RequestBody facebookAuthRequest: FacebookAuthRequest
    ): ResponseEntity<ApiResponse<AuthResponse>> {
    ...
    /** Refresh token endpoint (if needed for future implementation) POST /api/auth/refresh */
    @PostMapping("/refresh")
    fun refreshToken(
    ...
    /** Logout endpoint (for future implementation with token blacklisting) POST /api/auth/logout */
    @PostMapping("/logout")
    fun logout(
            @RequestHeader("Authorization") authHeader: String
    ): ResponseEntity<ApiResponse<String>> {
